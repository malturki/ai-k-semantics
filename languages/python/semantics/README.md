...
- adapter-backed range truthiness, equality, integer-like membership, indexing, and length
...
Current range caveat: adapter-backed `range` support is an internal `rangeVal` subset used by `for` loops, `len`, equality, truthiness, integer-like containment, and indexing, including positive and negative integer steps. Zero-step `ValueError`, range object attributes, slicing, non-integer containment fallback, out-of-range `IndexError`, and the general iterator protocol remain unsupported.
