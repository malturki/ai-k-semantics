...
- adapter-backed `complex()`, `complex(x)`, and `complex(real, imag)` for the current int/bool/float/complex subset
...
- adapter-backed complex values from imaginary literals, with truth-value testing, equality against complex/int/bool/float zero-imaginary values, unary `+`/`-`, and complex addition/subtraction/multiplication/nonzero true division with current int/bool/float/complex values
...
- adapter-backed flat, non-starred nested, starred, and nested starred sequence unpacking targets in `for` loops over current concrete string/bytes/list/tuple/dict/set/range outer iterables and item values
- adapter-backed `while`/`for` loop `else` clauses for the current loop subsets
- adapter-backed `try/finally` normal-completion, return-propagation, and `break`/`continue` propagation subset, including `break`/`continue` issued directly by the `finally` body, without `except` or `else`
