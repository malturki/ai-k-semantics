...
- adapter-backed flat, non-starred nested, and starred sequence unpacking targets in `for` loops over current concrete string/list/tuple/dict/set/range outer iterables, plus current string/range item values in supported unpacking targets
...
Current compound-statement caveat: the K parser does not yet accept Python indentation syntax directly. The AST adapter emits internal `#if`, `#while`, `#whileElse`, `#for`, `#forElse`, unpacking-`for`, and `#def` statements with explicit blocks for the supported subset. `for` iteration is defined only for current concrete string/list/tuple/dict/set/range values, with dictionaries iterating over keys and sets using the current internal set iteration order profile; flat and starred unpacking loop targets currently bind current concrete list/tuple/string/range item values, and non-starred nested loop targets bind current concrete list/tuple/string/set/range item values left to right. General iterator protocol, `try`, `with`, `match`, full function definitions, class definitions, and async compound statements remain unsupported.
