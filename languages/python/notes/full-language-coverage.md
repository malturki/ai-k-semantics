...
| Compound statements | Language Reference, chapter 8 | partial | Adapter-backed `if`, `while`, list/tuple/range `for`, and positional-parameter `def` subsets exist, including `break`, `continue`, and `return` propagation. General iteration, loop `else`, `try`, `with`, `match`, full functions, classes, coroutines, type parameter lists, annotations remain. |
...
| Assignment | partial | Single-name assignment, adapter-backed multi-target simple-name assignment, and simple-name augmented assignment for current integer-like operators; needs target lists, unpacking, attributes, subscriptions, annotations, `/=`, `@=`, and full target evaluation order. |
...
| Containers and comprehensions | partial | Minimal list/tuple/dict/set value displays, adapter-backed expression evaluation for list/tuple/dict/nonempty set displays, list/tuple equality, membership, truthiness, positive indexing, dict key membership, dict key lookup, and adapter-backed `range(stop)`/`range(start, stop)` values for `for` loops; direct K smoke uses trailing-comma input while adapter smoke accepts ordinary Python displays for the supported subset. Needs unpacking, mutation, slices, duplicate dict-key overwrite behavior, full range object behavior, iterators, and comprehensions. |
...
| Control flow | partial | Adapter-backed `if`, `while`, list/tuple/range `for`, `break`, `continue`, and function `return` subsets are covered, along with `pass`, truthy `assert`, and expression-level conditionals; needs general iteration, loop `else`, `yield`, context managers. |
