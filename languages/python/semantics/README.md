...
- adapter-backed `dict(x)` for current concrete dictionaries and current concrete list/tuple iterables of length-two list/tuple pairs, plus `dict(key=value)` and `dict(x, key=value)` over current simple keyword arguments without `**` unpacking
