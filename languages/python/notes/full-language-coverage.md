...
| Data model | Language Reference, chapter 3 | partial | Initial kernel has minimal integers, strings, list/tuple/dict values, booleans, and `None`. Full object identity, real bool-as-int subtype behavior, type hierarchy, attributes, descriptors, special methods, GC/lifetime notes, coroutines, and async generators remain. |
...
| Expressions | Language Reference, chapter 6 | partial | Initial kernel covers integer/string/list/tuple/dict/bool/None atoms, names, parentheses, unary `+`/`-`/`~`, arithmetic and bitwise integer operators, string concatenation/equality, list/tuple equality/membership/indexing, dict key membership/lookup, truthiness for simple values, short-circuit `and`/`or`, `not`, conditional expressions, single-argument lambdas/calls, and a binary comparison subset. All other expression constructs remain. |
...
| Truth-value testing | partial | Simple int/string/list/tuple/dict/bool/None truthiness and short-circuit `and`/`or`; needs `__bool__`, `__len__`, full containers, and user-defined objects. |
...
| Containers and comprehensions | partial | Minimal list/tuple/dict value displays, list/tuple equality, membership, truthiness, positive indexing, dict key membership, and dict key lookup; direct K smoke uses trailing-comma input while adapter smoke accepts ordinary Python displays for the supported subset. Needs full expression-list evaluation, mutation, slices, sets, ranges, iterators, and comprehensions. |
