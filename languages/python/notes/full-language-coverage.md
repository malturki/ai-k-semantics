...
| Containers and comprehensions | partial | Minimal list/tuple/dict/set value displays, list/tuple equality, membership, truthiness, positive indexing, dict key membership, and dict key lookup; direct K smoke uses trailing-comma input while adapter smoke accepts ordinary Python displays for the supported subset. Needs full expression-list evaluation, mutation, slices, ranges, iterators, and comprehensions. |
