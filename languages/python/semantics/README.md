...
- adapter-backed flat sequence unpacking assignment to simple names over current list/tuple/dict/set RHS values
- adapter-backed non-starred nested sequence unpacking assignment to simple names over current list/tuple/dict/set RHS values
- adapter-backed starred sequence unpacking assignment to simple names over current list/tuple/dict/set RHS values, with dictionaries yielding keys and sets using the current internal set iteration order profile
- integer literals
...
- adapter-backed `for` loops over the current string/list/tuple/dict/set value subset, including `break`, `continue`, and `return` propagation, with sets using the current internal set iteration order profile
- adapter-backed flat, non-starred nested, and starred sequence unpacking targets in `for` loops
...
Current compound-statement caveat: the K parser does not yet accept Python indentation syntax directly. The AST adapter emits internal `#if`, `#while`, `#whileElse`, `#for`, `#forElse`, unpacking-`for`, and `#def` statements with explicit blocks for the supported subset. `for` iteration is defined only for current concrete string/list/tuple/dict/set/range values, with dictionaries iterating over keys and sets using the current internal set iteration order profile; non-starred nested loop targets currently bind over current concrete list/tuple/dict/set outer values whose items are current concrete list/tuple/set values. General iterator protocol, `try`, `with`, `match`, full function definitions, class definitions, and async compound statements remain unsupported.
...
Current unpacking caveat: flat, non-starred nested, and starred assignment targets are adapter-backed for simple names and current concrete list/tuple/dict/set RHS values, with dictionaries yielding keys and sets using the current internal set iteration order profile, and flat, non-starred nested, and starred `for` targets are adapter-backed over current concrete list/tuple/dict/set outer iterables. A starred target receives a list of remaining values, possibly empty, and prefix/star/suffix name binding follows left-to-right target order; non-starred nested assignment and loop targets recursively bind left to right. General iterable unpacking, nested starred targets, unpacking diagnostics, and attribute/subscript targets remain unsupported.
