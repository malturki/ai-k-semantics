...
| Control flow | partial | Adapter-backed `if`, `while`, list/tuple/range `for`, loop `else`, `break`, `continue`, and function `return` subsets are covered, along with `pass`, truthy `assert`, and expression-level conditionals; needs general iteration, `yield`, context managers. |
