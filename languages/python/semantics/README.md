...
- adapter-backed function and lambda varargs parameters, including suffix defaults for fixed parameters and the no-default `*args, **kwargs` combination, collecting extra positional arguments into a tuple
- adapter-backed function and lambda `**kwargs` parameters in the no-default/no-keyword-only subset, collecting extra keyword arguments into a dictionary keyed by strings
