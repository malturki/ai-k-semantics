...
| Compound statements | Language Reference, chapter 8 | partial | Adapter-backed `if` and `while` subsets exist. `for`, loop `else`, `break`, `continue`, `try`, `with`, `match`, functions, classes, coroutines, type parameter lists, annotations remain. |
...
| Control flow | partial | Adapter-backed `if` and `while` subsets are covered, along with `pass`, truthy `assert`, and expression-level conditionals; needs `for`, loop `else`, `break`, `continue`, `return`, `yield`, context managers. |
