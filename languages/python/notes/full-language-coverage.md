...
| Compound statements | Language Reference, chapter 8 | partial | Adapter-backed `if` and `while` subsets exist, including `break` and `continue` through nearest `while`. `for`, loop `else`, `try`, `with`, `match`, functions, classes, coroutines, type parameter lists, annotations remain. |
