...
| --- | --- | --- | --- |
| Lexical analysis | Language Reference, chapter 2 | partial | Initial kernel uses semicolon-separated simple statements with trailing semicolons and parses integers, simple K strings, `True`, `False`, and `None`; full source encoding, physical/logical lines, indentation, comments, token classes, f/t-string details, bytes, and numeric literal edge cases remain. |
| Data model | Language Reference, chapter 3 | partial | Initial kernel has minimal integers, strings, list values, booleans, and `None`. Full object identity, real bool-as-int subtype behavior, type hierarchy, attributes, descriptors, special methods, GC/lifetime notes, coroutines, and async generators remain. |
...
| Expressions | Language Reference, chapter 6 | partial | Initial kernel covers integer/string/list/bool/None atoms, names, parentheses, unary `+`/`-`/`~`, arithmetic and bitwise integer operators, string concatenation/equality, list equality/membership/indexing, truthiness for simple values, short-circuit `and`/`or`, `not`, conditional expressions, and a binary comparison subset. All other expression constructs remain. |
...
| Numeric operations | partial | Int/bool unary `+`/`-`/`~`, `+`, `-`, `*`, `%`, `//`, shifts, bitwise operators, and nonnegative `**`; Python numeric tower, true division, negative exponent float behavior, full coercions, and error behavior remain. |
| Truth-value testing | partial | Simple int/string/list/bool/None truthiness and short-circuit `and`/`or`; needs `__bool__`, `__len__`, full containers, and user-defined objects. |
...
| Containers and comprehensions | partial | Minimal trailing-comma list value displays, list equality, membership, truthiness, and positive indexing; needs full expression-list evaluation, mutation, slices, tuples, dicts, sets, ranges, iterators, and comprehensions. |
