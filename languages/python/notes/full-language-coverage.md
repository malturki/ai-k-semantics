...
| Assignment | partial | Single-name assignment, adapter-backed multi-target simple-name assignment, and simple-name `+=`/`-=`/`*=`; needs target lists, unpacking, attributes, subscriptions, annotations, and all augmented assignment operators/target forms. |
