...
Current unpacking caveat: flat and starred assignment targets are adapter-backed for simple names and current concrete list/tuple/set RHS values, with sets using the current internal set iteration order profile, and flat/starred `for` targets are adapter-backed over current concrete list/tuple item values from current concrete list/tuple/set outer iterables. A starred target receives a list of remaining values, possibly empty, and prefix/star/suffix name binding follows left-to-right target order. General iterable unpacking, nested targets, unpacking diagnostics, and attribute/subscript targets remain unsupported.
