...
- adapter-backed `try/finally` normal-completion subset without `except`, `else`, `return`, `break`, or `continue` in the `try`/`finally` bodies
...
Current compound-statement caveat: the K parser does not yet accept Python indentation syntax directly. The AST adapter emits internal `#if`, `#while`, `#whileElse`, `#for`, `#forElse`, unpacking-`for`, normal-completion `#tryFinally`, and `#def` statements with explicit blocks for the supported subset. `for` iteration is defined only for current concrete string/list/tuple/dict/set/range values, with dictionaries iterating over keys and sets using the current internal set iteration order profile; supported unpacking loop targets bind current concrete string/list/tuple/dict/set/range item values left to right. `try/finally` currently runs the `finally` block after normal completion only; `except`, `else`, exceptions, and finally-over-`return`/`break`/`continue` remain unsupported. General iterator protocol, `with`, `match`, full function definitions, class definitions, and async compound statements remain unsupported.
