...
| Standard library interactions | partial | Internal `range(stop)`/`range(start, stop)`/`range(start, stop, step)` loop, equality, integer-like membership, truthiness, and length support exist; `len` works for current concrete values. Needs staged builtins/core library coverage, full range object behavior, zero-step diagnostics, general `__len__`, and host profile boundaries. |
