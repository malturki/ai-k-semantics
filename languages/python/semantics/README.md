...
- adapter-backed range truthiness, equality, integer-like membership, and length
...
Current range caveat: adapter-backed `range` support is an internal `rangeVal` subset used by `for` loops, `len`, equality, truthiness, and integer-like containment, including positive and negative integer steps. Zero-step `ValueError`, range object attributes, indexing, slicing, non-integer containment fallback, and the general iterator protocol remain unsupported.
