...
| Containers and comprehensions | partial | Minimal list/tuple/dict/set value displays, list/tuple equality, membership, truthiness, positive indexing, dict key membership, dict key lookup, and adapter-backed `range(stop)`/`range(start, stop)` values for `for` loops; direct K smoke uses trailing-comma input while adapter smoke accepts ordinary Python displays for the supported subset. Needs full expression-list evaluation, mutation, slices, full range object behavior, iterators, and comprehensions. |
